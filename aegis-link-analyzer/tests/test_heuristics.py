...
class TestHTTPScheme:
    ...
    def test_http_url_gets_higher_score_than_https(self):
        """
        HTTP URLs carry a penalty score (raised to 12 from 10 in heuristics fix).
        The http version must always score >= the https version of the same URL.
        """
        ...
        # HTTP penalty should be at least 12 (new minimum after score fix)
        assert http_score - https_score >= 12, (
            f"HTTP penalty should be >= 12, got difference {http_score - https_score}"
        )
...
# ════════════════════════════════════════════════════════════════
# Regression — Heuristic Score Minimums After Fix
# ════════════════════════════════════════════════════════════════

class TestHeuristicScoreMinimums:
    """
    After the scoring fix (TLD score 20→25, HTTP score 10→12), verify
    that the raw heuristic scores are at or above the new minimums.
    These tests catch regressions if the heuristics.py values are reverted.
    """

    def test_suspicious_tld_score_at_least_25(self):
        """TLD flag alone should contribute at least 25 to heuristic_score."""
        score = heuristic_score("https://example.xyz")
        assert score >= 25, (
            f"Suspicious TLD (.xyz) alone should produce heuristic_score >= 25, got {score}. "
            "Check SUSPICIOUS_TLD score in heuristics.py (should be += 25, not += 20)."
        )

    def test_http_score_at_least_12(self):
        """HTTP penalty should add at least 12 to the score."""
        https_score = heuristic_score("https://example.com")
        http_score  = heuristic_score("http://example.com")
        diff = http_score - https_score
        assert diff >= 12, (
            f"HTTP penalty should be >= 12, got {diff}. "
            "Check HTTP score in heuristics.py (should be += 12, not += 10)."
        )

    def test_brand_plus_keyword_score_at_least_43(self):
        """Brand typosquatting (35) + 1 keyword (8) = at least 43."""
        score = heuristic_score("https://paypal-secure.com/login")
        assert score >= 43, (
            f"Brand typosquat + keyword should score >= 43, got {score}."
        )
