...
def run_tests(host, verbose, skip_ollama, skip_network, section_filter, api_key=""):
    ...
    for k, v in feats.items():
        if isinstance(v, bool):
            print(f"    {'✓' if v else '○'} {k}")
    # Auto-detect: if Ollama is not ready on server, skip those tests
    if not skip_ollama and not feats.get("ollama_holistic_llm", False):
        skip_ollama = True
        print(f"  {YELLOW}○ Ollama not running — skipping LLM tests automatically{RESET}")
    ...
    if section("BLOCK 2 — Content & Language Intelligence", "block2"):
        ...
        run("AG-B2-008", "Posts: copy-paste detection",
            ...
            {"copy_paste_score__gt": 0.35}, tag="block2")
        ...
        run("AG-B2-011", "Links: shortener detected",
            ...
            {"links_analyzed__gte": 1,
             "link_details__len_gte": 1}, tag="block2",
...
if __name__ == "__main__":
    ...
    p.add_argument("--host",          default="http://localhost:8003")
    ...
    p.add_argument("--section",       default="",
                   help="Filter tests by section: block1|block2|block3|block4|unified")
    a = p.parse_args()
    run_tests(a.host, a.verbose, a.skip_ollama, a.skip_network, a.section, getattr(a, "api_key", "1122"))
