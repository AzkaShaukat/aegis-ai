...
async def explain_result(
    ...
    # Pick system prompt based on module for better context
    _MODULE_SYSTEMS = {
        "smishing": (
            _EXPLAIN_SYSTEM
            + "\n\nIMPORTANT: For smishing/SMS analysis use these words:\n"
            "- If HIGH RISK: 'This message is a scam/phishing attempt. Do NOT click any links or reply.'\n"
            "- If SAFE: 'This looks like a legitimate message. No scam signals detected.'\n"
            "- Always say Verdict: SCAM / SUSPICIOUS / LEGITIMATE\n"
            "- Always give Action: Report to FIA / Ignore / Proceed safely"
        ),
        "credential_leak": (
            _EXPLAIN_SYSTEM
            + "\n\nIMPORTANT: For leak/breach checks ONLY discuss breach exposure:\n"
            "- If breached: 'This email/number appeared in X hacked databases.'\n"
            "- If clean: 'This email/number has not been found in any leaked databases.'\n"
            "- NEVER mention scammers, fraud, or social media in leak check responses.\n"
            "- Verdict: BREACHED / CLEAN\n"
            "- Action: Change passwords / No action needed"
        ),
        "credential_scam": (
            _EXPLAIN_SYSTEM
            + "\n\nIMPORTANT: For scam checks ONLY discuss fraud/scam risk:\n"
            "- If scam risk: 'This email/number is linked to scam/fraud activity.'\n"
            "- If clean: 'No scam signals detected for this contact.'\n"
            "- NEVER mention data breaches or leaked databases in scam check responses.\n"
            "- Verdict: SCAMMER / SUSPICIOUS / SAFE TO CONTACT\n"
            "- Action: Block and report / Exercise caution / Safe to trust"
        ),
    }
    system = _MODULE_SYSTEMS.get(module, _EXPLAIN_SYSTEM)
    ...
    result_text = await _ask(prompt, system=system, max_tokens=250)
    if result_text:
        return result_text
