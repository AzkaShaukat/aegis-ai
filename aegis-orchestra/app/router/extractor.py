...
def extract(text: str, media_type: Optional[str] = None) -> ExtractedEntities:
    ...
    for em in ent.emails:
        if "@" in em:
            ...
            # Also add all parent domains (lgu.edu.pk → lgu.edu, edu.pk, lgu.edu)
            parts = domain.split(".")
    ...
    for m in _URL_RE.finditer(text):
        url = m.group(0)
        # Skip if this "URL" is actually just an email domain or subdomain thereof
        raw_domain = url.replace("https://","").replace("http://","").split("/")[0].lower()
        if raw_domain in email_domains:
            continue
        # Also skip if raw_domain is contained in any email domain
        if any(raw_domain in ed or ed in raw_domain for ed in email_domains):
            continue
