"""
app/services/deepfake_service.py
...
DEEPFAKE_URL = os.getenv("DEEPFAKE_SERVICE_URL", "http://localhost:8004")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(60.0)) as client:
            r = await client.post(
                ...
                json={"url": url, "source_hint": "whatsapp"},
            )
...
def format_deepfake_report(result: dict, is_video: bool = False, human_explanation: str = "") -> str:
    ...
    if message:
        ...
        lines.append(f"This image is {verdict.lower()} with {int(prob*100)}% confidence.")
    lines.append("")
    ...
    important_flags = [f for f in flags if not any(skip in f.lower() for skip in ["detected", "analyzing", "primary analyzed", "ensemble", "model", "agreement"])]
    if important_flags:
        lines.append("")
