...
# ============================================================================
# EXPANDED keyword sets for all intents
# ============================================================================
...
# ── Credential scan phrases (NEW) ──────────────────────────────────────────
_EMAIL_SCAN_PHRASES = {
    "scan this email", "scan my email", "scan email", "check this email",
    "check my email", "check email", "analyse this email", "analyze this email",
    "analyse email", "analyze email", "inspect this email", "inspect email",
    "is this email safe", "is my email safe", "email safe", "email security",
    "email breach", "email hacked", "email compromised", "email leaked",
    "check if this email is breached", "check if my email is breached",
    "is this email in a breach", "is my email in a breach",
    "i want to check my email", "please check my email",
    "can you check this email", "can you scan this email",
    "verify this email", "verify email", "email verification",
    "email analysis please", "email check please",
    "check this email address", "scan this email address",
    "perform email analysis", "run email check", "run email analysis",
    "do email check", "do email analysis", "email security check",
    "is my email compromised", "is my email safe to use",
    "check this email for breach", "check my email for leaks",
}

_PHONE_SCAN_PHRASES = {
    "scan this phone", "scan my phone", "scan phone", "check this phone",
    "check my phone", "check phone", "analyse this phone", "analyze this phone",
    "is this phone number safe", "is my phone safe", "phone breach",
    "phone hacked", "phone compromised", "phone leaked",
    "check if this phone number is breached", "is my phone number in a breach",
    "i want to check my phone", "please check my phone",
    "can you check this phone number", "verify this phone",
    "phone number verification", "phone analysis please",
    "check my phone number", "check this number", "is this number safe",
    "is my number safe", "phone security check", "number security check",
    "run phone check", "perform phone analysis", "check number for breach",
    "is this number compromised", "is my phone number compromised",
}

_PASSWORD_SCAN_PHRASES = {
    "scan this password", "scan my password", "scan password",
    "check this password", "check my password", "check password",
    "analyse this password", "analyze this password",
    "is this password safe", "is my password safe", "password strength",
    "password strong", "password weak", "is my password strong",
    "is my password weak", "how strong is this password",
    "check if this password is breached", "is this password breached",
    "i want to check my password", "please check my password",
    "can you check this password", "verify password",
    "password analysis please", "password check please",
    "test my password", "is it a good password",
    "check my password strength", "run password check",
    "perform password analysis", "security check password",
    "is my password secure", "password security check",
}

_CARD_SCAN_PHRASES = {
    "scan this card", "scan my card", "scan card", "check this card",
    "check my card", "check card", "credit card check", "debit card check",
    "is this card valid", "is my card valid", "card security",
    "check if this card is safe", "verify card", "card verification",
    "card analysis please", "card check please",
    "check credit card", "check debit card", "card number check",
    "validate card", "card validation", "is this card number valid",
    "check my credit card", "check my debit card",
}

_CNIC_SCAN_PHRASES = {
    "scan this cnic", "scan my cnic", "scan cnic", "check this cnic",
    "check my cnic", "check cnic", "cnic verification", "verify cnic",
    "is this cnic valid", "is my cnic valid", "cnic security",
    "check if this cnic is safe", "cnic analysis please",
    "national id check", "check national id",
    "check cnic number", "verify cnic number", "cnic validation",
    "is my cnic safe", "cnic security check", "run cnic check",
    "perform cnic analysis", "national identity card check",
}

_IBAN_SCAN_PHRASES = {
    "scan this iban", "scan my iban", "scan iban", "check this iban",
    "check my iban", "check iban", "iban verification", "verify iban",
    "is this iban valid", "is my iban valid", "iban security",
    "check if this iban is safe", "iban analysis please",
    "check iban number", "iban validation", "is this bank account valid",
    "verify bank account", "bank account check",
}

_CRYPTO_SCAN_PHRASES = {
    "scan this crypto address", "scan this wallet", "check crypto address",
    "check wallet", "verify crypto", "verify wallet",
    "is this crypto address safe", "is this wallet safe",
    "crypto wallet check", "bitcoin address check", "ethereum address check",
    "check crypto wallet", "verify bitcoin address", "verify ethereum address",
    "is my crypto safe", "crypto security check", "run crypto check",
    "perform crypto analysis", "check blockchain address",
}

_API_KEY_SCAN_PHRASES = {
    "scan this api key", "scan this token", "check api key", "check token",
    "verify api key", "verify token", "is this api key safe",
    "is this token safe", "api key check", "token check",
    "check my api key", "api key security", "run api key check",
    "perform api key analysis", "is this key valid",
}

_USERNAME_SCAN_PHRASES = {
    "scan this username", "check this username", "verify this username",
    "is this username safe", "username check", "username analysis",
    "check if this username is breached", "has this username been leaked",
    "check my username", "username security check", "run username check",
    "perform username analysis", "is my username safe",
}
...
_WHO_KEYWORDS = {
    ...
    "who designed you", "who programmed you", "what can you do",
    "how do you work", "what is your name", "your name",
    "aegis kya hai", "aegis kya hai ye", "ye kya hai",
}
...
_CYBER_QA_KEYWORDS = {
    ...
    "what is a strong password", "password tips", "security tips",
    "online security", "internet safety", "safe browsing",
    "what is a scam", "what is fraud", "how to avoid scams",
    "what is a data leak", "what is a security breach",
}
...
_CREDENTIAL_KEYWORDS = {
    ...
    # ── Added more ──
    "email compromise", "phone compromise", "password compromise",
    "identity theft", "check identity", "verify identity",
    "is my account safe", "account security", "check account",
    "my password safe", "my email safe", "my phone safe",
    "check credentials", "credential security", "security check",
    "is this credential safe", "credential check",
}
...
def classify(text: str, ent: ExtractedEntities, session: dict) -> RouteDecision:
    ...
    # ── Clear confirmation ──────────────────────────────────────
    if session.get("_clear_confirmed") and any(kw in text_lower for kw in {"yes", "y", "yeah", "yep", "confirm", "ok", "okay", "sure", "han", "haan"}):
    ...
    # ── Smishing fast-path ────────────────────────────────────────────────────
    # FIX-9: Run smishing BEFORE credential check so NADRA/government-impersonation
    # messages aren't hijacked by cnic/keyword detection. Only skip if real credentials
    # (emails, cards, actual CNICs) are extracted — not just text-word usernames.
    _has_real_cred_entities = (
        ...
        or ent.phone_numbers or ent.handles or ent.urls or ent.social_urls
    )
    ...
    if _has_keywords(text_lower, _PROFILE_KEYWORDS):
        ...
        if (not ent.handles and not ent.social_urls and
            any(w in text_lower for w in ["person","human","face","photo","picture","image"])):
    ...
    # ── Explicit credential scan phrases ──────────────────────────
    if any(kw in text_lower for kw in _EMAIL_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_email", raw_text=text)
    if any(kw in text_lower for kw in _PHONE_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_phone", raw_text=text)
    if any(kw in text_lower for kw in _PASSWORD_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_password", raw_text=text)
    if any(kw in text_lower for kw in _CARD_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_card", raw_text=text)
    if any(kw in text_lower for kw in _CNIC_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_cnic", raw_text=text)
    if any(kw in text_lower for kw in _IBAN_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_iban", raw_text=text)
    if any(kw in text_lower for kw in _CRYPTO_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_crypto", raw_text=text)
    if any(kw in text_lower for kw in _API_KEY_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_api_key", raw_text=text)
    if any(kw in text_lower for kw in _USERNAME_SCAN_PHRASES):
        return RouteDecision(primary=Module.CREDENTIAL, action="prompt_for_analyze_username", raw_text=text)
...
def _handle_disambiguation_response(text_lower: str, session: dict) -> RouteDecision:
    ...
    # backend/app/router/intent.py

# ── Clear confirmation ──────────────────────────────────────
    if session.get("_clear_confirmed") and any(kw in text_lower for kw in {"yes", "y", "yeah", "yep", "confirm", "ok", "okay", "sure", "han", "haan"}):
        return RouteDecision(primary=Module.SPECIAL, action="clear_session", raw_text=raw)
...
def _looks_like_smishing(text: str) -> bool:
    ...
    # FIX-9d: Standard multi-keyword scoring
    smish = {
        "otp", "pin", "verify", "account blocked", "urgent", "click",
