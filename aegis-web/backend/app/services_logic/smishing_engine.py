"""app/services_logic/smishing_engine.py — Dedicated Smishing Detection Engine.

Analyses suspicious SMS messages using:
1. Keyword + pattern scoring (immediate)
2. Ollama AI classification (primary)
3. Sub-analysis of any phone numbers found (credential service)
4. Sub-analysis of any links found (link analyzer)
...
async def analyse_smishing(
    ...
    # Step 2: Ollama AI analysis (if available)
    ol_score = 0
    ol_smish = False
    try:
        ...
        logger.debug("Ollama classify_smishing not available")
    ...
    if phone_numbers:
        try:
            from app.services.dispatcher import cred_analyze_phone
    ...
    # Step 6: Sub-analyse URLs if present
    if urls:
        try:
            from app.services.dispatcher import link_scan
            for url in urls[:2]:
    ...
    # Step 7: Plain explanation from Ollama (if available)
    if results["ollama_analysis"].get("reason"):
        results["plain_explanation"] = results["ollama_analysis"]["reason"]
    elif is_smishing:
        cat_map = {
        ...
        results["plain_explanation"] = cat_map.get(
            results["category"],
            "This message shows signs of being a scam."
        )
    else:
        results["plain_explanation"] = (
            "This message does not show strong scam indicators."
        )
...
def format_smishing_result(sms_text: str, analysis: Dict[str, Any]) -> str:
    """Format smishing analysis into a clear reply."""
    ...
    # Ollama AI reasoning
    ol_reason = ol_analysis.get("reason", "")
